void sendStatus() {
    if (mqtt.connected()) {
        ...
        snprintf(topic, sizeof(topic), "%s/%s", board_name, MQTT_STATUS_TOPIC);
...
void onMqttMessage(char *topic, char *payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index,
    ...
    if (cmd != nullptr && strcmp(topic, MQTT_UPGRADE_TOPIC) == 0) {
#if defined(HTTP_OTA)

        char topic[64] = {0};
        snprintf(topic, sizeof(topic), "%s/%s/%s", board_name, MQTT_UPGRADE_TOPIC,
    ...
    } else if (cmd != nullptr && strcmp(topic, MQTT_RESTART_TOPIC) == 0) {
        ESP.restart();
    }
}
...
void onMqttConnect(bool sessionPresent) { // callback
#if defined(DEBUG_ENABLED)
    ...
    // restart
    snprintf(topic, sizeof(topic), "%s/%s", board_name, MQTT_RESTART_TOPIC);
    mqtt.subscribe(topic, MQTT_QOS);

#if defined(HTTP_OTA)
    // upgrade
    snprintf(topic, sizeof(topic), "%s/%s", board_name, MQTT_UPGRADE_TOPIC);
    mqtt.subscribe(topic, MQTT_QOS);
#endif
...
void mqttSetup() {
    ...
    snprintf(will_topic, sizeof(will_topic), "%s/%s", board_name, MQTT_STATUS_TOPIC);
